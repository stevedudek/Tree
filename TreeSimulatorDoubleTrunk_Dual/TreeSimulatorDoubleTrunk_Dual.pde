...
void finishCycle(byte channel) {
  morph_frame(channel, 0.98);  // May work after all. ToDo: Test
...
color interp_color(color c1, color c2, float fract) {
 ...
 if (is_same_color(c1, c2)) {
   ...
   return color(hue(c2), 255, brightness(c2) * fract);
//   return color(hue(c2), saturation(c2), brightness(c2) * fract);
 } else if (is_black(c2)) {
   return color(hue(c1), 255, brightness(c1) * (1.0 - fract));
...
//   println();
   color new_color = color(interpolate_wrap(hue(c1), hue(c2), fract),
                255, // lerp(saturation(c1), saturation(c2), fract),  // 255 ?
