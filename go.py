...
class TreeServer(object):
    def __init__(self, tree_model, tree_simulator, args):
        self.args = args
    ...
    def _create_services(self):
        ...
        # Show runner
        self.runner = ShowRunner(self.tree_model, self.tree_simulator,
    ...
    def start(self):
        if self.running:
            print "start() called, but tree is already running!"
            return
        ...
        try:
            ...
            print "Exception starting Trees!"
            traceback.print_exc()
    ...
    def stop(self):
        if self.running: # should be safe to call multiple times
            try:
                ...
                print "Exception stopping Trees!"
                traceback.print_exc()
    ...
    def go_web(self, app):
        ...
        cherrypy.quickstart(TreeWeb(app),
                '/',
                config=config)

class TreeWeb(object):
...
if __name__=='__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Trees Light Control')
    ...
    print "Using Tree Simulator at %s:%d" % (sim_host, sim_port)

    from model.simulator import SimulatorModel

    model = SimulatorModel(sim_host, 0, port=sim_port)
    full_trees = tree.load_tree(model)

    app = TreeServer(full_trees, model, args)
    try:
        ...
        print "Unhandled exception running Trees!"
        traceback.print_exc()
    finally:
        app.stop()
